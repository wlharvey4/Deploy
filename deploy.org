...
#+TITLE:Deploy
#+AUTHOR:Pinecone062
#+DATE:<2019-04-21 Sun 13:54>
#+SUBTITLE:Guide on How to Deploy Websites Using a Variaety of Services @@html:<br>@@
#+SUBTITLE:Last Updated {{{date(%c)}}} by {{{author}}}

* Introduction
At its most fundamental, to deploy a web site on the Internet, one needs to
meet three conditions:

- content comprised of HTML, CSS, and JavaScript
- which is hosted on a server
- which is connected to the Internet via an IP address


Given those three conditions, other Internet users can access the content via a
...
server sends the content to the client, and the client displays the content
according to the instructions given by the HTML, CSS, and JavaScript.

This guide is devoted to exploring the second and third conditions.  The first
condition is all about web development, a huge topic all to itself.

This guide will explain not only the ``how'' of it, but also the ``why'' of
it.  It begins with a summary of the steps, with links to explanations
throughout.  Each level becomes more detailed the deeper you delve.
